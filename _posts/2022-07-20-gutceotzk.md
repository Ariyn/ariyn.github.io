...
{: .tractatus}
1. Go package의 함수 호출 관계를 보고 싶다면 `callgraph`와 `digraph`를 조합할 수 있다.

   1. 먼저 package의 call graph를 digraph 형식으로 만든다.

   2. 그 다음 관심 있는 package node만 추리고, 각 node가 호출하는 대상을 뽑는다.
...
2. 이렇게 만들면 함수별 호출 목록을 JSON에 가까운 형태로 정리할 수 있다.

   1. 시각화하거나, 특정 package 내부 호출만 따로 분석할 때 쓸 수 있다.

3. 단, call graph는 분석 방식에 따라 결과가 달라질 수 있다.

   1. interface dispatch, reflection, build tag가 얽히면 실제 런타임 호출과 완전히 같지 않을 수 있다.
